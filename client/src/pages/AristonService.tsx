...
export default function AristonService() {
  useEffect(() => {
    // Dynamic SEO Update
    document.title = "صيانة اريستون الإسكندرية | رقم صيانة Ariston المعتمد 01278885772";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) metaDesc.setAttribute("content", "هل تبحث عن صيانة اريستون بالإسكندرية؟ نحن مركز معتمد لصيانة كافة أجهزة Ariston (ثلاجات، غسالات، تكييفات) بالمنزل بقطع غيار أصلية وضمان سنة.");
    
    // Schema Markup for Google Ads Quality Score
    const schema = {
      "@context": "https://schema.org",
      "@type": "Service",
      "name": "صيانة اريستون الإسكندرية",
      ...
      "description": "خدمة صيانة اريستون المعتمدة في الإسكندرية بقطع غيار أصلية."
    };
      ...
      <section className="relative min-h-[75vh] flex items-center overflow-hidden bg-slate-900">
        <div className="absolute inset-0 z-0">
          <img 
            ...
            alt="صيانة اريستون المعتمدة بالإسكندرية" 
        ...
        <div className="container mx-auto px-4 relative z-10 py-12">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 bg-slate-500/20 border border-slate-500/30 px-4 py-2 rounded-full mb-6 animate-pulse">
              <ShieldCheck className="w-4 h-4 text-slate-400" />
              <span className="text-slate-400 font-bold text-sm uppercase tracking-wider">مركز صيانة Ariston المعتمد - الإسكندرية</span>
            </div>
            
            <h1 className="text-4xl md:text-7xl font-black text-white leading-tight mb-6">
              رقم صيانة <span className="text-slate-400">اريستون</span> <br />
            ...
            <p className="text-xl text-slate-200 mb-10 font-medium leading-relaxed max-w-2xl">
              نحن الخيار الأول لصيانة أجهزة Ariston في الإسكندرية. نغطي كافة المناطق (سموحة، لوران، العجمي، السيوف) ونصلك خلال 60 دقيقة.
            </p>
      ...
      <div className="bg-white py-16 border-b border-slate-100">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="p-6 bg-slate-50 rounded-3xl text-center group hover:bg-slate-600 transition-all duration-300">
              <Clock className="w-10 h-10 text-slate-600 mx-auto mb-4 group-hover:text-white" />
            ...
            <div className="p-6 bg-slate-50 rounded-3xl text-center group hover:bg-slate-600 transition-all duration-300">
              <ShieldCheck className="w-10 h-10 text-slate-600 mx-auto mb-4 group-hover:text-white" />
            ...
            <div className="p-6 bg-slate-50 rounded-3xl text-center group hover:bg-slate-600 transition-all duration-300">
              <Award className="w-10 h-10 text-slate-600 mx-auto mb-4 group-hover:text-white" />
            ...
            <div className="p-6 bg-slate-50 rounded-3xl text-center group hover:bg-slate-600 transition-all duration-300">
              <MapPin className="w-10 h-10 text-slate-600 mx-auto mb-4 group-hover:text-white" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div>
              <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-8 leading-tight">
                أفضل مركز صيانة <span className="text-slate-600">اريستون</span> <br />في الإسكندرية
              </h2>
              <p className="text-xl text-slate-600 mb-8 leading-relaxed font-medium">
                هل تعاني من أعطال في أجهزة Ariston؟ لا داعي للقلق. نحن نوفر لك خدمة صيانة منزلية فورية باستخدام أحدث معدات الفحص الإلكتروني لتحديد العطل بدقة متناهية وتوفير التكلفة.
              </p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
                {[
                  'صيانة ثلاجات اريستون',
                  'تصليح غسالات اريستون',
                  'صيانة تكييفات اريستون',
              ...
              <div className="bg-slate-600 p-10 rounded-[2.5rem] text-white shadow-2xl shadow-slate-900/20">
            ...
            <div className="relative">
              <div className="absolute -inset-4 bg-slate-600/10 rounded-[3rem] blur-3xl"></div>
              <img src="/images/services/washing-machine-repair-pro.jpg" alt="فني صيانة اريستون الإسكندرية" className="relative rounded-[3rem] shadow-2xl w-full h-[600px] object-cover" />
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {['سموحة', 'سيدي جابر', 'لوران', 'العجمي', 'السيوف', 'المنتزه', 'جليم', 'الإبراهيمية', 'محرم بك', 'سيدي بشر'].map(area => (
              <span key={area} className="px-6 py-3 bg-white/5 border border-white/10 rounded-full font-bold hover:bg-slate-600 transition-colors">
                صيانة اريستون {area}
              </span>
            ))}
          </div>
      ...
      <section id="contact" className="py-24 bg-slate-50">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="text-center mb-16">
            ...
            <div className="w-24 h-2 bg-slate-600 mx-auto rounded-full mt-6"></div>
          </div>
          <div className="bg-white p-2 rounded-[3rem] shadow-2xl border border-slate-200">
            <BookingForm defaultService="صيانة اريستون" />
          </div>
        </div>
      </section>
