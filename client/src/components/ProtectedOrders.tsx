...
export default function ProtectedOrders() {
  ...
  // ✅ دالة توزيع أرباح التاريخ (المصروفات لا تؤثر على الربح)
  const distributeProfitForDate = async (targetDate: string) => {
    try {
      ...
      // ✅ لا نطرح المصروفات هنا
      const netProfit = totalIncome;

      if (netProfit <= 0) {
        alert(`⚠️ لا توجد إيرادات (دخل) ليوم ${targetDate}.`);
        return;
      }
  ...
  const sendDailyReportToPartners = async (targetDate: string) => {
    try {
      ...
      const netProfit = totalIncome; // صافي الربح الموزع = الإيرادات فقط
...
✨ نظام إدارة الصيانة - تقرير يومي`;
  ...
  // دوال إنشاء وتعديل وحذف الأوردرات والفنيين والشركاء (نفس الكود الأصلي)
  // نظراً لطول الكود، تم حذفها للاختصار ولكن يمكنك إضافتها كما هي من ملفك الأصلي
          ...
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="mb-8 p-4 border rounded-lg">
              ...
              <div className="flex flex-wrap items-end gap-4">
                ...
                <button onClick={handleSendReportForDate} className="bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2">
