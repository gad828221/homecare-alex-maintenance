...
const sendExternalNotification = async (title: string, message: string) => {
  try {
    if (window.OneSignal && window.OneSignal.isPushNotificationsEnabled) {
      ...
      if (isEnabled) {
        await window.OneSignal.sendSelfNotification(title, message, {}, (completed: any) => {
          console.log('✅ إشعار خارجي مرسل:', title);
        });
      } else {
        console.log('⚠️ الإشعارات غير مفعلة من قبل المستخدم');
      }
    } else {
      console.log('⚠️ OneSignal غير جاهز بعد');
    }
  } catch (error) {
...
export default function ProtectedOrders() {
  ...
  // ✅ اختبار الإشعارات المنبثقة عند تحميل الصفحة
  useEffect(() => {
    const timer = setTimeout(() => {
      if (toastNotification) {
        toastNotification({
          ...
          message: 'الإشعارات المنبثقة تعمل بنجاح!',
          duration: 5000
        });
        console.log('✅ تم اختبار الإشعارات بنجاح');
      } else {
        console.error('❌ useNotification غير متوفر');
      }
    }, 3000);
      ...
      if (response.ok) {
        ...
        if (toastNotification) {
          toastNotification({
            type: 'success',
            title: '💰 إضافة أرباح',
            message: `تم إضافة ${companyShare} ج.م للخزنة من أوردر ${order.customer_name}`,
            duration: 4000
          });
        }
        return true;
...
✨ نظام إدارة الصيانة - تقرير يومي`;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200" dir="rtl">
      ...
      {/* مودالات (Order, Technician, Partner, Cash, Settlement) - نفس المودالات الموجودة */}
      {showOrderModal && (
