...
// ✅ دالة إرسال الإشعارات (نسخة مبسطة تستخدم sendSelfNotification)
const sendPushToExternalId = async (externalId: string, title: string, message: string) => {
  try {
    ...
    // التأكد من تسجيل الدخول بالمعرف المطلوب
    ...
    console.log(`✅ تم إرسال الإشعار إلى ${externalId}`);
    return true;
  } catch (err) {
    console.error(err);
    return false;
  ...
  const updateOrderStatus = async (id: number, newStatus: string, extraData = {}) => {
    ...
    try {
      ...
      if (newStatus === 'completed' || newStatus === 'in-progress') {
        ...
        if (order.technician) {
          await sendPushToExternalId(`tech_${order.technician}`, `🔧 تحديث`, `تم تغيير حالة أوردر ${order.customer_name}`);
        }
      }
    } catch (err) { console.error(err); }
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200" dir="rtl">
      ...
      <div className="flex gap-2 p-4 border-b bg-slate-900 overflow-x-auto">
        ...
        <button 
          onClick={async () => {
            if (window.OneSignal) {
              window.OneSignal.sendSelfNotification("🧪 زر مباشر", "هذه رسالة من زر التجربة", {}, () => alert("تم إرسال الإشعار المباشر"));
            } else {
              alert("OneSignal غير جاهز");
            }
          }}
          className="bg-pink-600 text-white px-3 py-2 rounded-lg text-sm"
        >
          🧪 إرسال مباشر (Self)
        </button>
      </div>
