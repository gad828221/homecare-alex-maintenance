...
export default function ProtectedOrders() {
  ...
  // حساب الرصيد: الإيرادات + الاحتياطي - المصروفات - توزيعات الأرباح
  ...
  // توزيع الأرباح: ربح اليوم = إجمالي الإيرادات فقط (بدون خصم المصروفات)
  const distributeProfitForDate = async (targetDate: string) => {
    try {
      ...
      const netProfit = totalIncome; // ربح اليوم = الإيرادات فقط

      if (netProfit <= 0) {
        alert(`⚠️ لا توجد أرباح (إيرادات) ليوم ${targetDate}.`);
        return;
      }
  ...
  const sendDailyReportToPartners = async (targetDate: string) => {
    try {
      ...
      const netProfit = totalIncome; // ربح اليوم = الإيرادات فقط (المصروفات لا تقلل منه)
...
✨ نظام إدارة الصيانة - تقرير يومي`;
  ...
  // دوال إدارة الأوردرات والفنيين والشركاء (نفس الكود الأصلي - محذوفة للاختصار)
  // يجب إضافة دوال createOrder, updateOrder, deleteOrder, createTechnician, updateTechnician, deleteTechnician, createPartner, updatePartner, deletePartner
  // نظراً لطول الكود، يمكنك إبقاؤها كما هي من ملفك الأصلي.
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gray-100">
      ...
      <div className="flex gap-2 p-4 border-b bg-white overflow-x-auto">
        ...
        <button onClick={() => setActiveTab('notifications')} className={`px-4 py-2 rounded-lg ${activeTab === 'notifications' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}>🔔 الإشعارات</button>
        {userRole === 'admin' && <button onClick={() => setActiveTab('permissions')} className={`px-4 py-2 rounded-lg ${activeTab === 'permissions' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}>🛡️ الصلاحيات</button>}
      ...
      <div className="p-4">
        {activeTab === 'cash' && (
          <div className="bg-white rounded-lg shadow p-4">
            <h2 className="text-xl font-bold mb-4">💰 إدارة الخزنة</h2>
            ...
            <div className="mb-8 p-4 border rounded-lg">
              ...
              <p className="text-sm text-gray-500 mb-3">⚠️ ملاحظة: يتم التوزيع على أساس إجمالي الإيرادات (الدخل) فقط، والمصروفات لا تؤثر على التوزيع.</p>
              <div className="flex flex-wrap items-end gap-4">
                <div>
                  <label className="block text-sm mb-1">اختر التاريخ</label>
                  <input type="date" value={selectedProfitDate} onChange={(e) => setSelectedProfitDate(e.target.value)} className="border p-2 rounded" />
                </div>
                <button onClick={handleDistributeSelectedProfit} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2">
                  <DollarSign size={18} /> توزيع أرباح التاريخ المحدد
                </button>
              </div>
            </div>
            ...
            <div className="mb-8 p-4 border rounded-lg">
              ...
              <div className="flex flex-wrap items-end gap-4">
                <div>
                  <label className="block text-sm mb-1">اختر التاريخ</label>
                  <input type="date" value={reportDate} onChange={(e) => setReportDate(e.target.value)} className="border p-2 rounded" />
                </div>
                <button onClick={() => sendDailyReportToPartners(reportDate)} className="bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2">
                  <Send size={18} /> إرسال تقرير التاريخ المحدد
                </button>
              </div>
            </div>

            <div className="mb-4 flex justify-between">
              <button onClick={() => { setEditingCash(null); setCashForm({ type: 'expense', amount: 0, description: '', date: new Date().toISOString().split('T')[0] }); setShowCashModal(true); }} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2"><Plus size={18} /> إضافة حركة</button>
              <div><input type="date" value={cashFilterDate} onChange={(e) => setCashFilterDate(e.target.value)} className="border p-2 rounded" placeholder="فلتر بالتاريخ" /></div>
            </div>
                ...
                <tbody>
                  {cashLedger.map(entry => (
                    <tr key={entry.id}>
                      ...
                      <td className="p-2 border">{entry.type === 'income' ? 'دخل' : entry.type === 'expense' ? 'مصروف' : entry.type === 'profit_distribution' ? 'توزيع أرباح' : 'رصيد احتياطي'}</td>
        ...
        {activeTab !== 'cash' && (
          <div className="bg-white p-4 rounded shadow">
            <p className="text-gray-500">(محتويات التبويبات الأخرى كما هي دون تغيير)</p>
          </div>
        )}
      </div>
