    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 pb-20">
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6">
        ...
        <div className="flex gap-1 bg-slate-900 p-1 rounded-2xl border border-slate-800 mb-6 w-fit">
          <button onClick={() => setActiveTab('orders')} className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'orders' ? 'bg-slate-800 text-white' : 'text-slate-500'}`}>الأوردرات</button>
          <button onClick={() => setActiveTab('technicians')} className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'technicians' ? 'bg-slate-800 text-white' : 'text-slate-500'}`}>الفنيين</button>
          <button onClick={() => setActiveTab('reports')} className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'reports' ? 'bg-slate-800 text-white' : 'text-slate-500'}`}>التقارير</button>
        </div>
              ...
              <div key={tech.id} className="bg-slate-900 rounded-3xl p-6 border border-slate-800 text-center relative">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    <a href={`tel:${tech.phone}`} className="flex-1 p-2 bg-blue-500/10 text-blue-500 rounded-xl hover:bg-blue-500/20 flex justify-center"><Phone className="w-4 h-4" /></a>
                    <button onClick={() => { setEditingTech(tech); setTechForm(tech); setShowTechModal(true); }} className="flex-1 p-2 bg-slate-800 text-slate-400 rounded-xl hover:bg-slate-700 flex justify-center"><Edit className="w-4 h-4" /></button>
                    <button onClick={() => deleteTechnician(tech.id)} className="flex-1 p-2 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500/20 flex justify-center"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="bg-slate-900 rounded-3xl p-8 border border-slate-800">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700">
                  ...
                  <p className="text-slate-400 text-xs font-bold mb-1">صافي أرباح الشركة</p>
