...
const BRANDS = ['سامسونج', 'LG', 'شارب', 'توشيبا', 'زانوسي', 'يونيون إير', 'فريش', 'وايت ويل', 'أريستون', 'بيكو', 'هوفر', 'إنديст'];
...
✨ نظام إدارة الصيانة - تقرير يومي`;
  ...
  // ✅ دالة updateOrderStatus مع إشعارات واتساب
  const updateOrderStatus = async (id: number, newStatus: string, extraData = {}) => {
    ...
    try {
      ...
      // إشعار للفني إذا كان معيناً (حسب الحالة)
  ...
  // ✅ دالة saveOrder مع إشعارات واتساب للمدير والفني
    ...
    try {
      if (editingOrder) {
        ...
        if (orderToSave.technician) {
          // إشعار للفني عبر واتساب (في حالة التعديل)
        ...
        // ✅ إشعار واتساب للفني إذا تم تعيينه
        if (orderToSave.technician) {
          const tech = technicians.find(t => t.name === orderToSave.technician);
          if (tech && tech.phone) {
  ...
  // ========== دوال التقارير (مثل السابق تماماً) ==========
