...
import { notifyTechnician, notifyAdmins, notifyNewOrder, notifyOrderStatusChange, notifyProfitAdded } from '../lib/onesignal';
...
// ✅ دالة إرسال الإشعارات عبر Netlify Function (معدلة)
const sendPushNotification = async (title: string, message: string) => {
  try {
    ...
    console.log('✅ Push sent via function:', result);
  } catch (err) {
...
export default function ProtectedOrders() {
  ...
  const [filterTechnicianReport, setFilterTechnicianReport] = useState<string>(''); // ✅ فلتر الفني في التقارير
  ...
  const addCompanyProfitToCash = async (order: any) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // ✅ إشعار تلقائي بإضافة أرباح
        await notifyProfitAdded(companyShare, `أوردر ${order.customer_name}`);
        return true;
...
✨ نظام إدارة الصيانة - تقرير يومي`;
  ...
  const updateOrderStatus = async (id: number, newStatus: string, extraData = {}) => {
    ...
    try {
      ...
      // ✅ إشعار تلقائي بتغيير الحالة للمديرين
      ...
      if (newStatus === 'completed' || newStatus === 'in-progress') {
        ...
        if (order.technician) {
          await sendPushNotification(`🔧 للفني ${order.technician}`, `تم تغيير حالة أوردر ${order.customer_name}`);
        }
      }
    } catch (err) { console.error(err); }
  };
  ...
  const saveOrder = async (e: React.FormEvent) => {
    ...
    try {
      if (editingOrder) {
        ...
        // ✅ إشعار تلقائي عند إضافة أوردر جديد
        await notifyNewOrder(orderToSave);
        
        await sendPushNotification('📋 أوردر جديد', `تم إضافة أوردر جديد للعميل ${formData.customer_name}`);
        if (orderToSave.technician) {
          await sendPushNotification(`🔧 للفني ${orderToSave.technician}`, `تم تعيين أوردر جديد لك: ${formData.customer_name}`);
        }
      }
  ...
  // ✅ تقرير الأوردرات غير المنفذة (مع فلتر الفني واستبعاد الاختبارات)
  ...
  // ✅ تقرير الأوردرات الملغية (مع فلتر الفني)
  ...
  const fetchProfitsReport = async () => {
    ...
    try {
      ...
      // استخراج أسماء الشركاء من وصف الحركة (مثلاً "توزيع أرباح: مصطفى جاد (25%)")
  ...
  // ✅ تقرير المقارنة (الإيرادات vs المصروفات vs الأرباح)
