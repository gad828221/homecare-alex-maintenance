              ...
              <table className="w-full text-right text-sm">
                <thead className="bg-slate-800/50 text-slate-300">
                  <tr>
                    ...
                    <th className="p-3">الإجمالي</th><th className="p-3">المصاريف</th><th className="p-3">الصافي</th><th className="p-3">نصيب الشركة</th><th className="p-3">نصيب الفني</th><th className="p-3">التاريخ</th><th className="p-3">إجراءات</th>
                  </tr>
                </thead>
                <tbody>
                  {orders.map(order => (
          ...
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setShowOrderModal(false)}>
            <div className="bg-slate-800 rounded-xl max-w-lg w-full p-6 border border-orange-500/30" onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={saveOrder} className="space-y-3">
                ...
                <input type="tel" placeholder="رقم الهاتف" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full p-2 rounded bg-slate-700 text-white" />
                ...
                <input type="text" placeholder="الفني" value={formData.technician} onChange={e => setFormData({...formData, technician: e.target.value})} className="w-full p-2 rounded bg-slate-700 text-white" />
                ...
                <input type="number" placeholder="الإجمالي" value={formData.total_amount} onChange={e => setFormData({...formData, total_amount: parseFloat(e.target.value)})} className="w-full p-2 rounded bg-slate-700 text-white" />
                <input type="number" placeholder="المصاريف" value={formData.expenses} onChange={e => setFormData({...formData, expenses: parseFloat(e.target.value)})} className="w-full p-2 rounded bg-slate-700 text-white" />
                <input type="number" placeholder="نصيب الشركة" value={formData.company_share} onChange={e => setFormData({...formData, company_share: parseFloat(e.target.value)})} className="w-full p-2 rounded bg-slate-700 text-white" />
                <input type="number" placeholder="نصيب الفني" value={formData.technician_share} onChange={e => setFormData({...formData, technician_share: parseFloat(e.target.value)})} className="w-full p-2 rounded bg-slate-700 text-white" />
                ...
                <div className="flex gap-2 mt-4">
                  <button type="submit" className="flex-1 bg-orange-600 text-white py-2 rounded-lg">حفظ</button>
                  <button type="button" onClick={() => setShowOrderModal(false)} className="flex-1 bg-slate-600 text-white py-2 rounded-lg">إلغاء</button>
                </div>
              </form>
            </div>
          </div>
            ...
            <div className="bg-slate-800 rounded-xl max-w-lg w-full p-6 border border-orange-500/30" onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={saveTechnician} className="space-y-3">
                ...
                <label className="flex items-center gap-2 text-white">
                  <input type="checkbox" checked={techForm.is_active} onChange={e => setTechForm({...techForm, is_active: e.target.checked})} /> نشط
                </label>
                <div className="flex gap-2 mt-4">
                  <button type="submit" className="flex-1 bg-orange-600 text-white py-2 rounded-lg">حفظ</button>
                  <button type="button" onClick={() => setShowTechModal(false)} className="flex-1 bg-slate-600 text-white py-2 rounded-lg">إلغاء</button>
                </div>
              </form>
            </div>
          </div>
