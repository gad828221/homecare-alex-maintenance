...
export default function ProtectedOrders() {
  ...
  // دالة توزيع الأرباح (بدون إضافة reserve)
  ...
  // إرسال التقرير مع رصيد الافتتاح والختام
...
✨ نظام إدارة الصيانة - تقرير يومي`;
  ...
  useEffect(() => {
    fetchData();
    // syncTechniciansToUsers(); // تم تعطيل المزامنة
  }, [fetchData]);
        ...
        {activeTab === 'orders' && (
          <div className="space-y-4">
            <div className="bg-slate-900 rounded-xl p-4 flex flex-wrap gap-3 items-center">
              ...
              <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} className="p-2 bg-slate-800 border border-slate-700 rounded-lg text-white"><option value="all">الكل</option><option value="pending">قيد الانتظار</option><option value="in-progress">قيد التنفيذ</option><option value="inspected">تم الكشف</option><option value="completed">مكتمل</option><option value="cancelled">ملغي</option><option value="deferred">مؤجل</option></select>
                  ...
                  return (
                    <div key={order.id} className={`bg-slate-900 rounded-xl border-r-4 p-4 ${delayed ? 'border-red-500' : order.status === 'completed' ? 'border-green-500' : order.status === 'in-progress' ? 'border-blue-500' : order.status === 'deferred' ? 'border-purple-500' : 'border-yellow-500'}`}>
                      ...
                      <div className="mt-2 text-xs text-slate-400">
                        {order.technician_note && <div>📝 ملاحظة الفني: {order.technician_note}</div>}
                      </div>
