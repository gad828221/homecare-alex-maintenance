...
export default function ProtectedOrders() {
  ...
  // تصحيح حساب الرصيد التراكمي: income و reserve يزيدان الرصيد، expense و profit_distribution ينقصانه
  ...
  const distributeProfitForDate = async (targetDate: string) => {
    try {
      // 1. جلب الإيرادات والمصروفات لهذا التاريخ فقط (بدون احتساب أرباح موزعة أو احتياطي)
      ...
      const netProfit = totalIncome - totalExpense; // صافي الربح قبل التوزيع
      ...
      // المبلغ الذي سيخرج من الخزنة ليتم توزيعه على الشركاء
      const amountToDistribute = (netProfit * totalPartnerShares) / 100;
      // المبلغ الذي سيبقى في الخزنة كرصيد احتياطي (يضاف إلى الخزنة)
      ...
      // --- التحقق من عدم وجود توزيع مسبق لنفس اليوم ---
      ...
      // --- توزيع الأرباح على الشركاء (تقلل من الخزنة) ---
      ...
      // --- إضافة الرصيد الاحتياطي (يزيد الخزنة) مع منع التكرار ---
  ...
  const sendDailyReportToPartners = async (targetDate: string) => {
    try {
      ...
      const netProfit = totalIncome - totalExpense; // صافي الربح قبل التوزيع والاحتياطي
...
✨ نظام إدارة الصيانة - تقرير يومي`;
  ...
  // باقي دوال إنشاء وتعديل وحذف الأوردرات والفنيين والشركاء (محذوفة للاختصار، يمكنك إضافتها من الكود الأصلي)
  // ... (ستحتفظ بالدوال الأخرى كما هي دون تغيير)
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-gray-100">
      ...
      <div className="p-4">
        {activeTab === 'cash' && (
          <div className="bg-white rounded-lg shadow p-4">
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg">
              <p className="text-lg">الرصيد الحالي: <span className="font-bold text-green-600">{cashBalance.toLocaleString()} ج.م</span></p>
            </div>
        ...
        {/* باقي التبويبات (orders, technicians, ...) يمكنك إضافتها من الكود الأصلي */}
        {activeTab !== 'cash' && <div className="bg-white p-4 rounded shadow">(محتويات التبويبات الأخرى كما هي)</div>}
      </div>
