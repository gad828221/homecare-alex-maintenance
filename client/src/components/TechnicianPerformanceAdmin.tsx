...
export default function TechnicianPerformanceAdmin({ orders, technicians }: { orders: any[]; technicians: any[] }) {
  ...
  const cards = useMemo(() => {
    const source = Array.isArray(technicians) && technicians.length
      ? technicians
      : Array.from(new Set((orders || []).map((order) => order?.technician).filter(Boolean))).map((name) => ({ name }));

    return source.map((technician, index) => {
      const techOrders = getTechnicianOrders(technician, Array.isArray(orders) ? orders : []);
  ...
  const totals = useMemo(() => cards.reduce((result, card) => {
    ...
    return result;
  }, { total: 0, completed: 0, cancelled: 0, inspected: 0, earnings: 0 }), [cards]);
      ...
      <section className="rounded-3xl border border-orange-500/25 bg-gradient-to-br from-slate-900 via-slate-900 to-orange-950/40 p-5 shadow-2xl">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
            ...
            <div className="col-span-2 rounded-xl bg-slate-800 px-3 py-2 text-center sm:col-span-1"><p className="text-[10px] text-slate-500">مستحقات</p><p className="text-sm font-black text-orange-300">{money(totals.earnings)}</p></div>
          </div>
        </div>
      </section>
        ...
        {cards.map((card) => {
          ...
          return (
            <article key={String(card.technician?.id || card.displayName)} className="overflow-hidden rounded-3xl border border-slate-700 bg-slate-900 shadow-xl transition hover:border-orange-500/50">
              ...
              <div className="grid grid-cols-2 gap-2 border-t border-slate-800 px-4 py-3 text-xs sm:grid-cols-4">
